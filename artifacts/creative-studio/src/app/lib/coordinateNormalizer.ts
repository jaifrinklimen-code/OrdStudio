 ...
 * 
 * CRITICAL FIX: Background elements are EXCLUDED from foreground bounding box calculations.
 * A 1920x1080 background rectangle must NEVER mask sub-scale foreground elements.
 */
...
export function normalizeCanonicalDesign(design: any): NormalizedDesignResult {
  ...
  const isPortraitDesign = (
    designCat.includes('letter') ||
    designCat.includes('resume') ||
    designCat.includes('poster') ||
    designCat.includes('flyer') ||
    designCat.includes('report') ||
    designName.includes('consent') ||
    designName.includes('letter') ||
    designName.includes('resume') ||
    designName.includes('flyer') ||
    (declaredW > 0 && declaredH > declaredW)
  );
  ...
  // 3. Check existing version flag: if design is already version 2, do not scale again
  const existingVersion = Number(design.coordinateVersion || design.canonicalCoordinateVersion || 1);
  ...
  // 4. Calculate FOREGROUND BOUNDS (excluding background)
  const fg = calculateForegroundBounds(rawEls, targetW, targetH);
  ...
  // 5. Detect legacy preview coordinate system
  ...
  if (targetW >= 1200 && targetW > targetH) {
    // Landscape 1920x1080 (or similar >= 1200)
  ...
  const scaleElement = (el: any) => {
    ...
    if (isBackgroundElement(el, targetW, targetH)) {
      // Background must NOT be scaled if already canonical
      // Set to canonical canvas dimensions
      scaled.x = 0;
      scaled.y = 0;
